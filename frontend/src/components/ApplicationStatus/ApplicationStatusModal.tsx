...
export default function ApplicationStatusModal({ open, onClose, application, onEdit, onCancel }: ApplicationStatusModalProps) {
  ...
  const contractMonths = application?.contractMonths || 6;
...
const accommodationPhotos = accomData?.imageUrls?.length > 0
  ...
  const cancelMutation = useMutation({
    mutationFn: async () => {
      ...
      if (application.applicationStatus === "approved" || application.applicationStatus === "waitlisted") {
        const res = await api.post(`/applications/${application.id}/confirm`, { action: "decline" });
        return res.data;
      } 
      else if (application.applicationStatus === "pending") {
        const res = await api.patch(`/applications/${application.id}`, { status: "cancelled" });
        return res.data;
      }
      else {
  ...
  const formattedRate =
    application.estimatedMonthlyRent !== null && application.estimatedMonthlyRent !== undefined
      ? new Intl.NumberFormat("en-PH", { style: "currency", currency: "PHP" }).format(Number(application.estimatedMonthlyRent))
      : "—";
  ...
  const isWaitlisted = application.applicationStatus === "waitlisted";
  ...
  const canCancel = isPending || isWaitlisted || (isApproved && hasRoomAssignment);
  const canEdit = isPending && !!onEdit;
  ...
  const modalFooter = (
    <div className="flex flex-col gap-3 w-full">
      ...
      <div className="flex justify-between items-center flex-wrap gap-2">
        <span className={`text-[10px] text-gray-400 max-w-[200px] leading-tight ${canCancel || canAccept ? 'text-wrap' : 'text-nowrap'}`}>
          {canCancel 
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {canCancel && onCancel && (
            <Button
              variant="danger"
              size="md"
              onClick={() => cancelConfirmation === "CANCEL" && onCancel()}
    ...
    <>
      <Modal open={open} onClose={onClose} title="APPLICATION STATUS" footer={modalFooter} maxWidth={650}>
        <div className="font-sans">
          <PhotoCarousel hidden={!carouselOpen} photos={accommodationPhotos}/>
