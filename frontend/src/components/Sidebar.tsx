...
function getActiveId(pathname: string, role: string): string {
  ...
  if (role === "landlord") {
    if (pathname.startsWith("/landlord/manage-accommodation")) return "dashboard";
...
export default function Sidebar({ role, profile }: SidebarProps) {
  ...
  const bottomItems: SidebarMenuItem[] = [
    { id: "account", label: "Account", icon: <ProfileIcon />, path: `/${role === "landlord-manage" ? "landlord" : role}/profile` },
