...
type Application = {
  ...
  stayType: "transient" | "non_transient"
  rejectionReason?: string | null
  applicationStatus: 'pending' | 'approved' | 'rejected' | 'cancelled' | 'waitlisted' | 'under_review' | 'confirmed'
  durationOfStayDays: number
  applicationDate: string
  applicationRoomType: string
  applicationStayType: string
  preferredTags?: string[]   // the student's selected tags
}
...
type Room = {
  ...
  tags?: { tagDetail: string }[]   // actual tags on the room
}
...
export default function ConfirmedStudents({ data, allRooms, onAssigned, className = "", baseUrl }: Props) {
  ...
  const openModal = (assignment: Assignment) => {
    if (assignment.status === "assigned") return   // prevent opening for assigned students
  ...
  // ---- Soft matching: filter rooms by hard criteria, then tag match status ----
  const matchedRooms = modalAssignment
    ? allRooms.filter((room) => {
        const app = modalAssignment.student
        // Hard filters: room type, stay type, tenant restriction
  ...
  // For each matched room, compute whether it is a perfect tag match
  const roomsWithMatchStatus = matchedRooms.map(room => {
    const app = modalAssignment!.student
    const preferredTags = app.preferredTags ?? []
    const roomTags = room.tags?.map(t => t.tagDetail) ?? []
    const allMatched = preferredTags.length === 0 || preferredTags.every(tag => roomTags.includes(tag))
    return {
      room,
      perfectTagMatch: allMatched
    }
  })
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <span className="text-[#1A0008] text-sm font-semibold capitalize">
                    {modalAssignment.student.applicationRoomType}
                  </span>
                </div>
              </div>

              {/* Room List with soft tag matching */}
              <div className="flex flex-col gap-3">
                {roomsWithMatchStatus.length > 0 ? (
                  roomsWithMatchStatus.map(({ room, perfectTagMatch }, i) => (
                    <div key={i} className="flex flex-col sm:flex-row sm:items-center gap-3 border border-[#F5ECF0] rounded-xl p-3 sm:p-4">
                      ...
                      <div className="flex flex-col gap-1 flex-1">
                        ...
                        {/* Tag match indicator */}
                        {modalAssignment.student.preferredTags && modalAssignment.student.preferredTags.length > 0 && (
                          <div className="mt-1">
                            {perfectTagMatch ? (
                              <span className="text-xs text-green-700 font-medium">✅ All amenities matched</span>
                            ) : (
                              <span className="text-xs text-amber-700 font-medium">⚠️ Partial amenity match</span>
                            )}
                          </div>
                        )}
                      </div>
                      <Button
                        variant="reddishPink"
                        size="sm"
                        disabled={room.roomCurrentOccupancy >= room.roomCapacity}
                        onClick={async () => {
                          const payload = {
                            ...
                            expectedMoveOut:
                              modalAssignment.expectedMoveOut ||
      ...
      <div className={className}>
        <div className="bg-white border border-[#E8D0D8] rounded-xl p-3 lg:p-4 shadow-sm w-full h-full flex flex-col">
          <div className="flex flex-row justify-between w-full pb-2 border-b border-[#F5ECF0]">
            <p className="text-[#1A0008] font-bold">Confirmed Students</p>
                    ...
                    <div key={i} className="grid grid-cols-5 items-center mt-3"
                      ...
                      <div className="flex flex-col px-2">
                        ...
                        <p className="text-[#9A7080] text-xs">
                          {assignment.roomBuilding ? `${assignment.roomBuilding} · ${assignment.roomType}` : 'Not assigned'}
                        </p>
                      </div>
                      <div className="flex flex-col px-2">
                      ...
                      <div className="col-span-1 px-2 flex justify-center items-center">
                        {assignment.status === "not assigned" ? (
                          ...
                          <span className="text-[#1A7A4A] text-sm font-semibold">Assigned</span>
                        )}
                      </div>
                    </div>
                  ))}
