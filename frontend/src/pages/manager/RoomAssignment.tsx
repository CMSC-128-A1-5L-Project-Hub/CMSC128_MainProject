...
// ── Modal Components (reuse the same as before but now they receive RawRoom[]) ──
          ...
          {roomsWithMatchStatus.length > 0 ? (
            roomsWithMatchStatus.map(({ room, perfectMatch }, i) => (
              <div key={i} className={`flex flex-col sm:flex-row sm:items-center gap-3 border rounded-xl p-3 sm:p-4 ${
                perfectMatch && studentPrefs.length > 0 ? 'border-green-200 bg-green-50/50' : 'border-[#F5ECF0]'
              }`}>
...
export default function RoomAssignment() {
  ...
  return (
    <>
      <Modal open={!!selectedAssignment} onClose={() => setSelectedAssignment(null)}
      ...
      <div className="flex h-screen bg-[#F5EEF0] font-sans">
        ...
        <div className="flex-1 flex flex-col px-8 py-5 overflow-y-auto">
          ...
          <main className="flex-1 flex flex-col gap-3.5">
            <HeroBanner greeting="Good Day" name={fullName} title="Assign a room to your confirmed applicant"
              subtitle="We make it easy for you to track the accommodation applications you manage." type="mini" />

            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              <Card>
                <div className="flex flex-col gap-4">
                  <StatBar label="Assigned" value={totalAssigned} total={allAssignments.length}
                    from="#1A7A4A" to="#2D9A5F" bg="#F0F7F3" textColor="#1A7A4A" />
                  <hr className="border-[#F5EEF0] my-1" />
                  <StatBar label="Pending" value={totalUnassigned} total={allAssignments.length}
                    from="#C9973A" to="#E0B040" bg="#FDF6E8" textColor="#C9973A" />
                </div>
              </Card>

              <div className="grid grid-cols-2 gap-4 lg:col-span-2">
                <DonutStatCard id="available-rooms" title="Available Rooms"
                  titleClassName="text-[#1A7A4A] text-sm lg:text-lg font-semibold uppercase"
                  value={totalRooms - occupiedRooms} total={totalRooms}
                  subtitle={`of ${totalRooms} total`} subtitleClassName="text-lg"
                  color={["#1B7C4B", "#2C995E"]} trackColor="#DCEBE3"
                  donutSize={120} strokeWidth={13} valueSize="3xl" valueWeight="extrabold" pctSize="xl" />
                <DonutStatCard id="occupied-rooms" title="Occupied Rooms"
                  titleClassName="text-[#6B0F2B] text-sm lg:text-lg font-semibold uppercase"
                  value={occupiedRooms} total={totalRooms}
                  subtitle={`of ${totalRooms} total`} subtitleClassName="text-lg"
                  color={["#72112D", "#921C3A"]} trackColor="#F1E0E6"
                  donutSize={120} strokeWidth={13} valueSize="3xl" valueWeight="extrabold" pctSize="xl" />
              </div>
            </div>
                ...
                <div className="flex items-center justify-between mb-2 gap-3 flex-wrap border-b border-[#F5ECF0] pb-2">
                  ...
                  <div className="flex items-center gap-2 ml-auto">
                    ...
                    <div className="flex items-center gap-2 border border-[#E8D5DC] rounded-xl px-3 py-2 bg-white">
                      ...
                      <input type="text" placeholder="Search assignments..." value={search} onChange={handleSearch}
                  ...
                  <div className="min-w-[900px]">
                    <div className="grid border-b border-[#F5ECF0]" style={{ gridTemplateColumns: GRID_COLS }}>
                      {["Student", "Confirmed Room Type", "Date Confirmed", "Status", "Action"].map((h, i) => (
                        <p key={h} className={`text-[#9A7080] text-xs font-bold p-1 uppercase ${i === 3 || i === 4 ? "text-center" : ""}`}>{h}</p>
                      ))}
                    </div>
                      ...
                      <>
                        <div className="flex flex-col">
                          {paginatedAssignments.map((assignment, i) => (
                            <div key={i} className={`grid items-center py-2.5 pl-1 ${assignment.status === "assigned" ? "bg-[#1A7A4A]/5" : ""}`}
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <p className="font-bold text-[12px] lg:text-sm text-[#1A0008]">{assignment.student.student.fullName}</p>
                              </div>
                              <div className="flex flex-col">
                              ...
                              <div className="flex justify-center">
                                <Button variant="reddishPink" size="sm" className="w-28 justify-center"
