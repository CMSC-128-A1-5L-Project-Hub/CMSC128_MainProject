      ...
      <div className="relative h-40 sm:h-48 overflow-hidden">
        <img
          ...
          className={`w-full h-full object-cover transition-transform duration-500 ${
            !isUnderReview ? "group-hover:scale-110" : ""
          }`}
        ...
        <div className="absolute top-2 sm:top-3 left-2 sm:left-3">
          {isUnderReview ? (
            <span className="px-2 sm:px-3 py-0.5 sm:py-1 rounded-full text-[9px] sm:text-[10px] font-bold bg-yellow-100 text-yellow-700 border border-yellow-200">
              Under Review
            </span>
          ) : (
            <span className="px-2 sm:px-3 py-0.5 sm:py-1 rounded-full text-[9px] sm:text-[10px] font-bold bg-emerald-100 text-emerald-700 border border-emerald-200">
              Verified
            </span>
          )}
        </div>
        ...
        <div className="absolute top-2 sm:top-3 right-2 sm:right-3">
          <span className="px-2 sm:px-3 py-0.5 sm:py-1 rounded-full text-[9px] sm:text-[10px] font-bold bg-white/90 text-[#6B0F2B] backdrop-blur-sm">
            {typeLabel}
          </span>
        </div>
        ...
        <div className="absolute bottom-2 sm:bottom-3 left-2 sm:left-3 right-2 sm:right-3">
          <h3 className="text-white font-bold text-base sm:text-lg leading-tight drop-shadow-lg truncate">
      ...
      <div className="p-3 sm:p-4">
        <div className="flex items-center gap-3 sm:gap-4 mb-3 sm:mb-4 text-[10px] sm:text-xs text-gray-500">
          <div className="flex items-center gap-1">
            ...
            <span>{accommodation.accommodationCapacity} capacity</span>
          </div>
          <div className="flex items-center gap-1">
            <Building2 size={12} className="sm:w-[13px] sm:h-[13px]" />
            <span className="capitalize">{accommodation.accommodationType?.replace('_', ' ')}</span>
          </div>
        </div>
        ...
        {accommodation.tags?.length > 0 && (
          <div className="flex flex-wrap gap-1 sm:gap-1.5 mb-3 sm:mb-4">
            {accommodation.tags.slice(0, 4).map((tag: any) => (
              <span key={tag.id || tag.tagDetail} className="px-1.5 sm:px-2 py-0.5 rounded-full text-[9px] sm:text-[10px] font-medium bg-[#8C1535]/5 text-[#8C1535] border border-[#8C1535]/10">
                {tag.tagDetail}
              </span>
            ))}
            {accommodation.tags.length > 4 && (
              <span className="px-1.5 sm:px-2 py-0.5 rounded-full text-[9px] sm:text-[10px] font-medium text-gray-400">
                +{accommodation.tags.length - 4}
              </span>
            )}
          </div>
        ...
        {/* Button - always shows but only works when verified */}
        <button
          onClick={(e) => {
            e.stopPropagation()
            handleClick()
          ...
          className={`w-full flex items-center justify-center gap-1.5 sm:gap-2 py-2 sm:py-2.5 rounded-xl font-bold text-xs sm:text-sm transition-all duration-200 ${
            isUnderReview 
              ...
              : "bg-gradient-to-r from-[#6B0F2B] to-[#8C1535] text-white hover:from-[#8C1535] hover:to-[#6B0F2B] shadow-md hover:shadow-lg"
          }`}
        >
          {isUnderReview ? (
            <>
            ...
            <>
              Manage <ArrowRight size={13} className="sm:w-[15px] sm:h-[15px]" />
            </>
          )}
        </button>
      </div>
    ...
    {isUnderReview && (
      <div className="absolute inset-0 bg-[#7a001f]/80 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
        <div className="text-center px-4">
          ...
          <span className="bg-white text-[#7a001f] text-[10px] sm:text-xs font-semibold px-3 sm:px-4 py-1.5 sm:py-2 rounded-full shadow">
...
const StepOne = ({ onNext, amenities, setAmenities }: {
  ...
  const {
    accommodationName, accommodationType, tenantRestriction,
    accommodationCapacity, businessPermit, latitude, longitude,
    setField, setBusinessPermit,
    ...
    <div className="space-y-3 sm:space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
        ...
        <div className="flex flex-col gap-1">
          ...
          <input type="number" value={accommodationCapacity} onChange={(e) => setField("accommodationCapacity", e.target.value)}
            className="border border-[#e5cfd4] rounded-xl p-2.5 sm:p-3 text-xs sm:text-sm w-full focus:outline-none focus:ring-2 focus:ring-[#7a001f]/30" placeholder="e.g., 20" />
          ...
          <div className="flex flex-wrap gap-1 sm:gap-1.5">
            {amenities.map((name) => (
              <span key={name} className="inline-flex items-center gap-1 sm:gap-1.5 pl-2 sm:pl-2.5 pr-1 sm:pr-1.5 py-1 sm:py-1.5 bg-emerald-600 text-white rounded-full text-[9px] sm:text-xs font-medium">
                ...
                <button type="button" onClick={() => setAmenities(amenities.filter(a => a !== name))}
                  className="w-3.5 h-3.5 sm:w-4 sm:h-4 flex items-center justify-center hover:bg-emerald-700 rounded-full transition"><X size={10} className="sm:w-3 sm:h-3" /></button>
              </span>
            ))}
          </div>
        )}
            ...
            {COMMON_AMENITIES.map((name) => {
              ...
              return (
                <button key={name} type="button"
                  onClick={() => setAmenities(isSelected ? amenities.filter(a => a !== name) : [...amenities, name])}
      ...
      <div className="flex flex-col gap-1">
        ...
        <div
          className={`flex flex-col items-center justify-center gap-3 border-2 border-dashed rounded-2xl p-4 sm:p-6 cursor-pointer transition ${dragOver ? "border-[#C9973A] bg-[#C9973A]/5" : "border-[#6B0F2B]/20 bg-white"}`}
          ...
          <div className="text-center">
            <p className="font-semibold text-sm sm:text-base text-[#1a1a1a]">Upload Business Permit</p>
            <p className="text-[10px] sm:text-xs text-gray-400 mt-0.5 uppercase">.PDF, .JPG, .PNG. Max 5MB</p>
          </div>
          {businessPermit ? (
...
// ─── STEP 2 ──────────────────────────────────────────────────────────────────
const StepTwo = ({ onBack, onClose, amenities }: { onBack: () => void; onClose: () => void; amenities: string[]; }) => {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="grid grid-cols-1 xs:grid-cols-2 gap-3 sm:gap-4">
        ...
        <div
          className={`border-2 border-dashed rounded-xl h-28 sm:h-32 flex flex-col items-center justify-center cursor-pointer transition ${dragOver ? "border-[#C9973A] bg-[#C9973A]/5" : "border-[#6B0F2B]/20 bg-white"}`}
          onClick={() => fileInputRef.current?.click()}
          onDragOver={(e) => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)}
      ...
      <div className="flex justify-between gap-3 sm:gap-4">
        ...
        <Button onClick={() => mutate()} variant="primary" size="sm" disabled={isPending || images.length < 4}>
          {isPending ? <><div className="w-3.5 h-3.5 sm:w-4 sm:h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />Uploading...</> : "Submit"}
        </Button>
      </div>
    </div>
  );
...
const ManageAccommodationDashboard: React.FC = () => {
  ...
  const handleSuccessClose = () => { setOpen(false); setStep(1); reset(); setAmenities([]); refetch(); };

  if (isLoading) return (
    <div className="flex items-center justify-center min-h-screen bg-[#f5f5f5]">
      <div className="text-center"><div className="w-10 h-10 border-4 border-[#8C1535] border-t-transparent rounded-full animate-spin mx-auto mb-4" /><p className="text-gray-500 font-medium">Loading...</p></div>
    </div>
  );
  if (isError) return null;

  return (
    <div className="flex flex-col lg:flex-row bg-[#FAF8F9] min-h-screen">
      ...
      <div className="flex-1 p-4 sm:p-6 lg:p-10 mt-12 lg:mt-0">
        <div className="text-center mb-6 sm:mb-10">
          ...
          <h1 className="text-3xl sm:text-5xl lg:text-6xl font-['Cormorant_Garamond'] font-semibold text-[#1a0a0f] tracking-tight leading-[0.85]">
        ...
        <div className="grid grid-cols-3 gap-3 sm:gap-4 mb-6 sm:mb-8 max-w-md mx-auto">
          {[
            { value: (accommodations ?? []).length, label: "Properties", color: "text-[#8C1535]" },
            { value: (accommodations ?? []).filter((a: any) => a.status === 'verified').length, label: "Verified", color: "text-emerald-600" },
            { value: (accommodations ?? []).filter((a: any) => a.status === 'pending').length, label: "Pending", color: "text-amber-600" },
          ].map((stat) => (
      ...
      <Modal open={open} onClose={handleClose}
        ...
        {step === 1 ? (
          ...
          <StepTwo onBack={() => setStep(1)} onClose={handleSuccessClose} amenities={amenities} />
        )}
      </Modal>
    </div>
  );
