...
export default function MapPage() {
  ...
  return (
    ...
    >
      <motion.div layoutId="map" className="w-full h-full" transition={{ type: "spring", stiffness: 140, damping: 25, mass: 0.8 }}>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {(showAllTags ? availableTags : availableTags.slice(0, 10)).map(tag => (
                          <button
                            ...
                            className={`px-3 py-1.5 text-[10px] font-semibold rounded-full border transition-all ${selectedTags.includes(tag)
