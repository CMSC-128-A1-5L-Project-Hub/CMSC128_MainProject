...
export default class RoomsController {
  ...
  async store({ params, request, auth, serialize }: HttpContext) {
    ...
    return serialize({ message: 'Room added successfully.', data: room.serialize() })
  }
  ...
  async update({ params, request, response, serialize }: HttpContext) {
    try {
      ...
      return serialize({ message: 'Room updated successfully.', data: updatedRoom.serialize() })
    } catch (err) {
  ...
  async reportIssue({ auth, params, request, response }: HttpContext) {
    ...
    if (!issueDetails) return response.badRequest({ message: 'Issue details are required' })

    await LogService.record(
      user.id, 'room', room.id, 'ROOM_ISSUE_REPORTED',
    ...
    const landlord = (room as any).accommodation?.landlord
