...
export default class extends BaseSchema {
  async up() {
    this.schema.createTable('fee', (table) => {
    ...
    this.schema.createTable('payment', (table) => {
      table.increments('id').primary()
      table.integer('fee_id').unsigned().references('id').inTable('fees')
      table.timestamp('timestamp', { useTz: true }).defaultTo(this.now())
      table.decimal('amount', 10, 2).notNullable()
      table.string('mode_of_payment', 30).notNullable()
    })

    this.schema.createTable('report', (table) => {
      table.increments('id').primary()
      table.integer('landlord_id').unsigned().references('user_id').inTable('landlords')
      table.string('student_number').references('student_number').inTable('students')
